...
class UserValidation{
    ...
    public static function isLessonDatetimeAvailable($lessonStartTime, $lessonFinishTime, $instructorLogin) {
        ...
        try {
            $result = $db->mysqli->query(
                sprintf("SELECT les.startDate, les.finishDate FROM lesson les "
                      . "INNER JOIN account a ON les.idInstructor=a.idAccount "
                      . "WHERE a.login='%s' AND DATE(les.startDate)=DATE('%s')",
                mysqli_real_escape_string($db->mysqli, $instructorLogin),
                mysqli_real_escape_string($db->mysqli, $lessonStartTime)));
            while($row = $result->fetch_assoc()) {
                ...
                if($userFinishTimeUnix > $instructorStartLessonUnix && $userStartTimeUnix < $instructorFinishLessonUnix) {
                    $_SESSION['bookLessonValidation'] = 'Proszę podać datę dostępną dla wybranego instruktora! '
                                                      . 'Zobacz grafik instruktora klikając w wybrany dzień w kalendarzu.';
                    return false;
                }
            }
        } catch (Exception $ex) {
