...
class UnloggedPerson {
    public function login() {
        ...
        $result = $db->mysqli->query(
                sprintf("SELECT * FROM account WHERE login='%s'", 
    ...
    public static function insertNewUser($firstname, $surname, $birthdate, $email, $login, $pass) {
        ...
        try {
            $db->mysqli->query(
                sprintf("INSERT INTO account VALUES(NULL, '%s', '%s', 'Użytkownik', '%s', NULL)", 
                mysqli_real_escape_string($db->mysqli, $login),
                mysqli_real_escape_string($db->mysqli, $passHash),
                mysqli_real_escape_string($db->mysqli, $creationDate)));  
            $idAccount = $db->mysqli->query(
                sprintf("SELECT idAccount FROM account WHERE login='%s'", 
                mysqli_real_escape_string($db->mysqli, $login)));
            $row = $idAccount->fetch_assoc();
            $db->mysqli->query(
                sprintf("INSERT INTO personaldata VALUES('%s', '%s', '%s', '%s', '%s')", 
                mysqli_real_escape_string($db->mysqli, $row['idAccount']),
                ...
                mysqli_real_escape_string($db->mysqli, $birthdate)));
            $db->mysqli->commit();
