...
class Statistic {
    ...
    public function getWorkerHoursByMonths() {
        ...
        try {
            $result = $db->mysqli->query(
                    "SELECT SUM(UNIX_TIMESTAMP(ws.finishTime)-UNIX_TIMESTAMP(ws.startTime)) AS unixSeconds, "
                    . "a.role, YEAR(ws.startTime) AS year, MONTH(ws.startTime) AS month FROM workschedule ws "
                    . "INNER JOIN account a ON ws.idUser=a.idAccount WHERE ws.startTime<NOW() "
                    . "GROUP BY YEAR(ws.startTime), MONTH(ws.startTime), a.role ORDER BY YEAR(ws.startTime), MONTH(ws.startTime)");
            while($row = $result->fetch_assoc()) {
    ...
    public function getTicketHighlights() {
        $all['oneHour'] = 0; $all['twoHours'] = 0; $all['threeHours'] = 0; $all['allDay'] = 0;
        $db = new Database();
        try {
            $result = $db->mysqli->query("SELECT UNIX_TIMESTAMP(finishTime)-UNIX_TIMESTAMP(startTime) as unixSeconds "
                                       . "FROM skipass WHERE isCancelled=0");
            while($row = $result->fetch_assoc()) {
    ...
    public function getHireHighlights() {
        ...
        $all["helmet"] = 0;
        try {
            $result = $db->mysqli->query("SELECT COUNT(*) as countValue, e.type FROM hire h "
                                       . "INNER JOIN hire_has_equipment hhe ON h.idHire=hhe.idHire "
                                       . "INNER JOIN equipment e ON e.idEquipment=hhe.idEquipment "
                                       . "WHERE h.status='Zakończono' GROUP BY e.type");
            while($row = $result->fetch_assoc()) {
               switch($row['type']) {
