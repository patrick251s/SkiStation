...
class Admin{
    public function addWorker() {
        ...
        if(Validation::firstnameValidation($firstname) && Validation::surnameValidation($surname) && 
           Validation::birthdateValidation($birthdate) && Validation::emailValidation($email) && 
           Validation::roleValidation($role) && Validation::instructorValidation($role, $checkboxSkiing, $checkboxSnowboard) && 
           Validation::loginValidation($login) && Validation::passwordValidation($pass, $pass2)) {
    ...
    public function deletePerson() {
        $personId = filter_input(INPUT_POST, 'workerId');
        ...
        try {
            $db->mysqli->query(
                ...
                mysqli_real_escape_string($db->mysqli, $personId)));
            $db->mysqli->query(
                sprintf("UPDATE skipass SET isCancelled = 1 WHERE idUser = '%s' AND startTime > NOW()",
                mysqli_real_escape_string($db->mysqli, $personId)));
            $db->mysqli->query(
                sprintf("UPDATE lesson SET isCancelled = 1 WHERE idUser = '%s' AND startDate > NOW()",
                mysqli_real_escape_string($db->mysqli, $personId)));
            $db->mysqli->query(
                sprintf("DELETE FROM workschedule WHERE idUser = '%s' AND startTime > NOW()",
                mysqli_real_escape_string($db->mysqli, $personId)));
            $db->mysqli->commit();
        } catch (Exception $ex) {  
            $db->mysqli->rollback();
    ...
    public function submitCalendar() {
        ...
        if(Validation::dayValidation($day, $isOpen, $startTimeUnix, $finishTimeUnix)) {
            if(Validation::isDayInDatabase($day)) {
                if($isOpen == 1) {
    ...
    public static function updateDay($day, $isOpen, $startTime, $finishTime) {
        ...
        try {
            $db->mysqli->query(
                ...
                mysqli_real_escape_string($db->mysqli, $day)));
            $db->mysqli->query(
                sprintf("UPDATE lesson SET cost=0, isCancelled=1 "
                      . "WHERE DATE(startDate)='%s' AND (startDate < '".$startDatatime."' OR finishDate > '".$finishDatatime."')",
                mysqli_real_escape_string($db->mysqli, $day))); 
            $db->mysqli->query(
                sprintf("UPDATE skipass SET cost=0, isCancelled=1 "
                      . "WHERE DATE(startTime)='%s' AND (startTime < '".$startDatatime."' OR finishTime > '".$finishDatatime."')",
                mysqli_real_escape_string($db->mysqli, $day))); 
            $db->mysqli->commit();
            $_SESSION['addCalendar'] = true; 
        } catch (Exception $ex) {
    ...
    public static function insertNewTimetable($login, $startTime, $finishTime) {
        ...
        try {
            $result = $db->mysqli->query(
                        sprintf("SELECT idAccount FROM account "
                              . "WHERE login = '%s' AND role IN ('Instruktor', 'Serwisant', 'Techniczny')",
                        mysqli_real_escape_string($db->mysqli, $login)));
