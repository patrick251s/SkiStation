...
function makeWorkerHoursDiagram(queryResult) {
    ...
    for(let i=0; i<Object.keys(queryResult.instructor).length; i++) {
        arrayToVisualization.push([Object.keys(queryResult.instructor)[i], 
            parseFloat(queryResult.instructor[Object.keys(queryResult.instructor)[i]]), 
            parseFloat(queryResult.technician[Object.keys(queryResult.technician)[i]]), 
            parseFloat(queryResult.serviceman[Object.keys(queryResult.serviceman)[i]])]);
    }
    function drawChart() {
      var data = google.visualization.arrayToDataTable(arrayToVisualization);
      var options = {
...
function makeMostActiveInstructorDiagram(queryResult) {
    ...
    function drawChart() {
      ...
      var options = {
        ...
        hAxis: {title: 'Liczba udzielonych lekcji / uczestników'},
