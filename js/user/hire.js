...
function displayUserHireList(data) {
    ...
    if(data.startTime.length === 0) {
        ...
        HTML += '<div class="table-responsive-lg"><table class="table">\n\
                <thead><tr id="firstTr"><th scope="col">Dzień</th><th scope="col">Rozpoczęcie</th>\n\
                <th scope="col">Zakończenie</th><th scope="col">Sprzęt</th><th scope="col">Zdjęcie</th>\n\
                <th scope="col">Koszt</th><th scope="col">Status</th></tr></thead><tbody>';
        let tempIdHire = '';
        
        for(let i=0; i<data.idHire.length; i++) {
            if(data.idHire[i] === tempIdHire) continue;
            if(data.finishTime[i] !== null) finishTime = data.finishTime[i].substring(11, 16);
            if(data.cost[i] !== null) cost = data.cost[i];

            HTML += '<tr><td>'+data.startTime[i].substring(0, 10)+'</td><td>'+data.startTime[i].substring(11, 16)+'</td>\n\
                    <td>'+finishTime+'</td><td>'+getHireEquipment(data, i)+'</td><td>'+getHirePhotos(data, i)+'</td>\n\
                    <td>'+cost+' zł</td><td>'+data.status[i]+'</td></tr>';
